...
transparency()
{
  wxPdfDocument pdf;
  pdf.AddPage();
  pdf.SetFont(_T("Arial"),_T(""),16);

  int j;
  wxString txt;
  for (j = 0; j < 180; j++) txt += _T("Hello World! ");
  ...
  int maskImg = pdf.ImageMask(_T("mask.png"));

  // embed image, masked with previously embedded mask
  pdf.Image(_T("image.png"), 55, 10, 100, 0, _T("png"), wxPdfLink(-1), maskImg);
  ...
  pdf.Image(_T("image_with_alpha.png"), 55, 190, 100);
  ...
  pdf.SetAlpha();
  pdf.SetFillColor(wxPdfColour(255,0,0));
  ...
  // draw green square
  pdf.SetFillColor(wxPdfColour(0,255,0));
  pdf.Rect(20, 20, 40, 40, wxPDF_STYLE_FILLDRAW);

  // draw jpeg image
  pdf.Image(_T("lena.jpg"), 30, 30, 40);
  ...
  pdf.Text(46,68,_T("Lena"));

  pdf.SaveAsFile(_T("transparency.pdf"));
}
